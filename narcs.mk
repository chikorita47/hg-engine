...
SHADOWOFFSETS_DIR := $(BUILD)/poke_shadow_ofx
SHADOWOFFSETS_NARC := $(BUILD_NARC)/poke_shadow_ofx.narc
SHADOWOFFSETS_TARGET := $(FILESYS)/poketool/pokegra/poke_shadow_ofx.narc
SHADOWOFFSETS_DEPENDENCIES := armips/data/shadowoffsets.s

$(SHADOWOFFSETS_NARC): $(SHADOWOFFSETS_DEPENDENCIES)
	$(PYTHON) $(NARCHIVE) extract $(SHADOWOFFSETS_TARGET) -o $(SHADOWOFFSETS_DIR) -nf
	$(ARMIPS) $^
	$(PYTHON) $(NARCHIVE) create $@ $(SHADOWOFFSETS_DIR) -nf

NARC_FILES += $(SHADOWOFFSETS_NARC)


SHADOWSIZES_DIR := $(BUILD)/poke_shadow
SHADOWSIZES_NARC := $(BUILD_NARC)/poke_shadow.narc
SHADOWSIZES_TARGET := $(FILESYS)/poketool/pokegra/poke_shadow.narc
SHADOWSIZES_DEPENDENCIES := armips/data/shadowsizes.s

$(SHADOWSIZES_NARC): $(SHADOWSIZES_DEPENDENCIES)
	$(PYTHON) $(NARCHIVE) extract $(SHADOWSIZES_TARGET) -o $(SHADOWSIZES_DIR) -nf
	$(ARMIPS) $^
	$(PYTHON) $(NARCHIVE) create $@ $(SHADOWSIZES_DIR) -nf
